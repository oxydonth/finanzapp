...
export default function DashboardPage() {
  ...
  const tooltipStyle = theme === 'dark'
    ? { borderRadius: '12px', border: 'none', boxShadow: '0 4px 12px rgb(0 0 0 / 0.4)', fontSize: '12px', background: '#1e293b', color: '#e2e8f0' }
    : { borderRadius: '12px', border: 'none', boxShadow: '0 4px 12px rgb(0 0 0 / 0.1)', fontSize: '12px' };
    ...
    <div className="p-8 max-w-7xl mx-auto animate-fade-in">
      ...
      <div className="grid grid-cols-2 gap-5 mb-5">
        ...
        <div className="card p-6">
          ...
          <ResponsiveContainer width="100%" height={200}>
            <BarChart data={flowData} barGap={3}>
              ...
              <Tooltip contentStyle={tooltipStyle} formatter={(v: number) => formatEUR(v * 100)} />
        ...
        <div className="card p-6">
          <div className="flex items-center justify-between mb-5">
            ...
            <Link href="/budget" className="flex items-center gap-1 text-xs font-medium text-brand-600 hover:text-brand-700 dark:text-brand-400 dark:hover:text-brand-300 transition-colors">
          ...
          <div className="space-y-4">
              ...
              return (
                <div key={b.id}>
                  <div className="flex justify-between text-sm mb-1.5">
                    <span className="text-slate-700 dark:text-slate-300 font-medium">{b.category?.icon} {b.name}</span>
            ...
            {budgetList.length === 0 && (
              <p className="text-sm text-slate-400">
                ...
                <Link href="/budget" className="text-brand-600 dark:text-brand-400">{t('dashboard.createBudget')}</Link>
              </p>
            )}
          </div>
        </div>
      ...
      <div className="card p-6 mb-5">
        <div className="flex items-center justify-between mb-5">
          ...
          <Link href="/accounts" className="flex items-center gap-1 text-xs font-medium text-brand-600 hover:text-brand-700 dark:text-brand-400 dark:hover:text-brand-300 transition-colors">
            {t('dashboard.showAll')} <ArrowUpRight size={12} />
          </Link>
        </div>
        <div className="grid grid-cols-2 gap-3">
          {accounts.slice(0, 4).map((a) => (
            <div key={a.id} className="border border-slate-100 dark:border-slate-800 rounded-xl p-4 hover:border-slate-200 dark:hover:border-slate-700 hover:shadow-sm transition-all duration-150">
              <p className="text-xs text-slate-400 dark:text-slate-500 mb-0.5">{a.accountName}</p>
              <p className="text-lg font-bold text-slate-900 dark:text-slate-100 tracking-tight">{formatEUR(Number(a.balanceCents))}</p>
              <p className="text-xs text-slate-400 dark:text-slate-500 font-mono mt-1">{a.ibanMasked}</p>
            </div>
          ))}
          {accounts.length === 0 && (
            <div className="col-span-2 text-center py-8 text-slate-400 text-sm">
              <Link href="/banks/connect" className="text-brand-600 hover:text-brand-700 dark:text-brand-400">{t('dashboard.connectBank')}</Link>
            </div>
          )}
        </div>
      </div>
      ...
      <div className="card p-6">
        <div className="flex items-center justify-between mb-5">
          ...
          <Link href="/transactions" className="flex items-center gap-1 text-xs font-medium text-brand-600 hover:text-brand-700 dark:text-brand-400 dark:hover:text-brand-300 transition-colors">
            {t('dashboard.showAll')} <ArrowUpRight size={12} />
          </Link>
        </div>
        <div className="space-y-1">
          {transactions.map((tx) => (
            <div key={tx.id} className="flex items-center justify-between py-2.5 px-3 -mx-3 rounded-xl hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
              ...
              <span className={`text-sm font-semibold tabular-nums ${tx.type === 'CREDIT' ? 'text-emerald-600 dark:text-emerald-400' : 'text-slate-900 dark:text-slate-100'}`}>
